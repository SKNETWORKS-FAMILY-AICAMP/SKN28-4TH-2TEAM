...
이 모델은 **4개 주제 영역(subject area)** 으로 나뉩니다. 하나의 큰 도표 대신
...
| **C. KAIST 기본 정보** | department_office, kaist_profile, kaist_statistics, kaist_link | KAIST 전체/행정/공식 링크 정보 |
| **D. RAG 검색** | rag_document, rag_chunk | 검색·임베딩용 텍스트 |
...
erDiagram
    department_office {
        varchar office_id PK "사무실 ID"
        varchar program_name "학과/프로그램명"
        varchar phone "전화번호"
        varchar website "웹사이트"
        varchar building_location "건물 위치"
    }
    kaist_profile {
        varchar item PK "항목명"
        text    content "내용"
    ...
    kaist_statistics {
        varchar stat_group PK "통계 그룹"
        varchar level PK "구분"
        varchar value_raw "원문 값"
        bigint  value_number "숫자 값"
    }
    kaist_link {
        varchar link_name PK "링크명"
        varchar url "URL"
        varchar note "비고"
    }
```

> `department_office`는 KAIST 전체 모집요강의 학과사무실 표에서 온 데이터입니다.
> AI대학 4개 학과 마스터와 범위가 다르므로 `department` FK를 걸지 않았습니다.
...
> **정규화 정리(3NF)**: `rag_chunk` 는 CSV 원본상 `dept·source_type·title·source_url·`
> `source_board·crawled_at` 를 함께 갖지만, 이는 모두 `doc_id` 에만 종속되어
> `rag_document` 와 전수 중복(이행적 종속)입니다. → 최종 테이블에서는 제거했습니다.
...
4. **주제 영역 분리** — 업무 / 수집·자원 / KAIST 기본 정보 / RAG 를 나눠, 영역 간 결합도를 낮추고 도표 가독성을 확보.
...
| KAIST 기본 | department_office | office_id (VARCHAR) | — | 41 | 학과사무실/행정실 연락처 |
| KAIST 기본 | kaist_profile | item (VARCHAR) | — | 9 | KAIST 기본 정보 |
| KAIST 기본 | kaist_statistics | (stat_group, level) | — | 12 | KAIST 통계 |
| KAIST 기본 | kaist_link | link_name (VARCHAR) | — | 7 | 공식 링크 |
| RAG | rag_document | doc_id (VARCHAR) | dept | 577 | 문서 메타 |
| RAG | rag_chunk | chunk_id (VARCHAR) | doc_id | 778 | 검색용 청크(PDF 본문 포함, 문서메타 제거) |
