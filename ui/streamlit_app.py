...
# Windows hnswlib/chromadb OpenMP 크래시 방지
...
from ui.components.layout import render_source_cards
from rag.query_analyzer import DEPARTMENTS
from rag.rag_pipeline import RagPipeline, create_default_pipeline
...
# 인라인 CSS: styles.py에서 통합 관리하므로 별도 override 불필요
...
html(
    ...
    '<div class="feature-panel-title">What Users Can Do</div>'
...
if not st.session_state.pending_user_question and not st.session_state._answer_in_progress:
    st.session_state.is_processing = False
...
def is_all_departments_followup(question: str) -> bool:
    keywords = ["다", "전체", "전부", "모두", "모든", "각 학과", "학과별", "네 개", "4개"]
...
def run_for_all_departments(question: str, pending_question: str = ""):
    pipeline = get_pipeline()
    answers = []
    all_sources = []
    all_warnings = []

    combined_question = " ".join(part for part in [pending_question, question] if part)
...
def should_answer_all_departments(question: str, pending: dict | None) -> bool:
    ...
    return analysis.intent != "general_info" and "department" in analysis.missing_fields
...
def build_answer_payload(question: str):
    ...
    if should_answer_all_departments(question, pending):
        ...
        cache_key = build_cache_key(
            question=" ".join(part for part in [pending_question, question] if part),
...
def queue_question(question: str) -> None:
    ...
    st.session_state.pending_user_question = clean_question
...
# =========================
st.markdown(
    '<div class="section-title" style="font-size:1.5rem; margin-top:2rem; margin-bottom:0.6rem;">💬 Chat</div>',
...
with st.form(
    key=f"chat_form_{st.session_state.input_counter}",
